...
So, the obvious takeaway is that the refractive index $n$ increases with frequency everywhere (normal dispersion) \emph{except} in the immediate vicinity of a resonance, where it temporarily decreases (anomalous dispersion). The anomalous region is also exactly where absorption is strongest. This is not a coincidence — both effects share the same physical root: the driven oscillators are completely out of phase with the driving field near $\omega_0$, so the medium simultaneously extracts energy from the wave (absorption) and distorts its phase velocity (anomalous dispersion).
