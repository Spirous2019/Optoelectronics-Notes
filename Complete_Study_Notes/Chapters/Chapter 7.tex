...
At a forward-biased single heterojunction, both electrons and holes are injected across the interface, but in general one carrier type dominates. The ratio of the electron current to the hole current depends on both the doping levels and the bandgap difference between the two sides:
\begin{equation*}
    \left|\frac{J_e}{J_h}\right| = \frac{D_e L_h}{D_h L_e}\cdot\frac{N_D}{N_A}\cdot\exp\!\left(\frac{E_{g,n} - E_{g,p}}{k_B T}\right)
\end{equation*}
where $E_{g,n}$ and $E_{g,p}$ are the bandgaps on the n- and p-sides respectively. The exponential factor dominates: even a modest bandgap difference of $\sim 0.1\,\text{eV}$ (compared to $k_BT \approx 26\,\text{meV}$ at room temperature) exponentially suppresses the unwanted carrier current. The \textbf{injection efficiency} — the fraction of total current carried by the desired carrier type — is:
\begin{equation*}
...
Combining generation, spontaneous and non-radiative recombination, and stimulated emission (where $S$ is the photon density, $G$ is the stimulated emission rate per unit photon density, and $\Gamma$ is the optical confinement factor):
\begin{keyresult}
\begin{equation}
...
\begin{itemize}
    \item $J/(ed)$: the carrier generation rate from the external drive current — every milliamp of current injects a fixed number of electron-hole pairs per second into the active volume.
    \item $N/\tau$: the total spontaneous and non-radiative recombination loss — carriers draining away as wasted heat or as spontaneous (incoherent) photons that do not contribute to the lasing mode.
    \item $\Gamma\,G\,S$: the stimulated depletion — coherent photons in the lasing mode consuming carriers to generate more coherent photons. The $\Gamma$ factor accounts for the fact that only the fraction of the optical mode overlapping the active layer drives stimulated transitions.
\end{itemize}
